    ...
    for x in range(numsamples):
        ...
        for i in range(N_new + 1):
            for j in range(N_new + 1):
                local_energies[x] += 20 * np.absolute(
                    newcode.plaquette_defects[i, j] - oldcode.plaquette_defects[i, j]) + np.sum(samples[x, :])
...
class CustomRNNCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        preact = tf.matmul(inputs, self._weights) + tf.matmul(state, self._recurrent_weights) + tf.matmul(upleftstate,
                                                                                                          self._upleft_weight) + tf.matmul(
            uprightstate, self._upright_weight) + tf.matmul(downleftstate, self._downleft_weight) + tf.matmul(
            downrightstate, self._upleft_weight)
...
class DilatedRNNWavefunction(object):
    def __init__(self, systemsize, cell=None, activation=tf.nn.relu, units=[2], scope='DilatedRNNwavefunction'):
        ...
        with self.graph.as_default():
            with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
                ...
                self.rnn = [
                    [cell(num_units=units[i], activation=activation, name="rnn_" + str(n) + str(i), dtype=tf.float64)
                     for n in range(self.N)] for i in range(self.numlayers)]
                self.dense = [tf.compat.v1.layers.Dense(2, activation=tf.nn.softmax, name='wf_dense' + str(n)) for n in
                              range(self.N)]  # Define the Fully-Connected la
    ...
    def sample(self, numsamples, inputdim):
            ...
            with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
                ...
                for i in range(self.numlayers):
                    for n in range(self.N):
                        ...
                        rnn_states.append(self.rnn[i][n].zero_state(self.numsamples,
                                                                    dtype=tf.float64))  # Initialize the RNN hidden state
                ...
                for n in range(self.N):
                    ...
                    for i in range(self.numlayers):
                        if (n - 2 ** i) >= 0:
                            rnn_output, rnn_states[i + n * self.numlayers] = self.rnn[i][n](rnn_output, rnn_states[
                                i + ((n - 2 ** i) * self.numlayers)])  # Compute the next hidden states
                        else:
                            rnn_output, rnn_states[i + n * self.numlayers] = self.rnn[i][n](rnn_output, rnn_states[i])

                    output = self.dense[n](rnn_output)
                    probs.append(output)
                    sample_temp = tf.reshape(tf.compat.v1.multinomial(tf.math.log(output), num_samples=1),
                                             [-1, ])  # Sample from the probability
        ...
        self.samples = tf.stack(values=samples,
                                axis=1)  # (self.N, num_samples) to (num_samples, self.N): Generate self.numsamples vectors of size self.N spin containing 0 or 1
        ...
        with self.graph.as_default():
            ...
            with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
                ...
                for n in range(self.N):
                    ...
                    output = self.dense[n](rnn_output)
                    probs.append(output)
                    inputs = tf.reshape(tf.one_hot(tf.reshape(
                        tf.slice(samples, begin=[np.int32(0), np.int32(n)], size=[np.int32(-1), np.int32(1)]),
                        shape=[self.numsamples]), depth=self.outputdim, dtype=tf.float64),
                                        shape=[self.numsamples, self.inputdim])
            ...
            self.log_probs = tf.reduce_sum(tf.math.log(tf.reduce_sum(tf.multiply(probs, one_hot_samples), axis=2)),
                                           axis=1)
...
if __name__ == '__main__':
    ...
    with tf.compat.v1.variable_scope(RNNWF.scope, reuse=tf.compat.v1.AUTO_REUSE):
        with RNNWF.graph.as_default():
            ...
            start = time.time()
            tempfailrate = 1
            for it in range(len(meanEnergy), num_steps + 1000):
    ...
    for p_idx in range(n_ps):
        ...
        for samp in range(ndatapoints):
            ...
            for i in range(numsamples):
                temparray = samples[0, :].reshape(3, 3)
